...
> **Mandatory V3 design reference:** ก่อนออกแบบ แก้ token หรือเลือก semantic role ต้องอ่าน [`../../../DESIGN.md`](../../../DESIGN.md) และยึดเอกสารนั้นเป็น source of truth ด้าน design language, visual rules, token intent, typography, spacing, radius, effects และ component variants ของ Wi Design System V3 เอกสารนี้ยังคงเป็น source of truth ด้าน Flutter architecture, token generation และ runtime API

ลำดับการตีความสำหรับงาน Theme V3:

1. `DESIGN.md` กำหนด design intent และกฎการใช้งาน
2. `tokens/**` เป็น editable implementation source ที่ generator อ่านจริง
3. `generated/**` เป็น derived output ห้ามแก้ด้วยมือ
4. หาก `DESIGN.md` ไม่ตรงกับ token JSON หรือ runtime API ให้หยุด ระบุความต่าง และแก้ source ให้สอดคล้องผ่าน token workflow ห้าม hardcode ค่าใน widget หรือเลือกค่าใกล้เคียงเอง
...
- อ่านและตรวจ mapping กับ `DESIGN.md` ก่อนแก้ Theme V3
...
### `tokens/primitive/primitive.tokens.json`
...
### `tokens/semantic/radius.json` และ `tokens/semantic/space.json`

Editable sources สำหรับ theme-invariant semantic dimensions โดย radius มี 9 รายการและ spacing มี 18 รายการ ทุก token ต้องมี `com.figma.aliasData.targetVariableName` ชี้ไปยัง primitive ที่มีอยู่ และ resolved number ต้องตรงกับ primitive target

### `tokens/semantic/typography.json`

Editable source สำหรับ typography 18 semantic styles ในกลุ่ม Display, Heading, Label และ Paragraph ตัว parser resolve font definition ภายในไฟล์, map weight เป็น Flutter `FontWeight` และแปลง absolute line-height เป็น `TextStyle.height = lineHeight / fontSize`

### `tokens/primitive/shadow.effect.json`

Editable source สำหรับ shadow geometry 10 layers ซึ่ง compose เป็น 6 scales ได้แก่ `sm`, `base`, `md`, `lg`, `xl` และ `xl2` สีทุก layer ต้อง alias ไปยัง `color.alpha.json` และ resolved color ต้องตรงกับ primitive target Shadow ยังเป็น primitive scale เพราะ source ไม่มี semantic role mapping สำหรับ card/dropdown/modal
...
1. อ่าน primitive colors/dimensions และ semantic colors/dimensions JSON
2. parse และ validate token schema
3. ตรวจ Light/Dark path parity
4. resolve semantic aliases
5. สร้าง primitive Dart constants
6. สร้าง semantic Light/Dark palettes และ semantic dimension constants
...
```text
Theme V3 generated: primitives=145, alphaColors=52, radii=9, spacing=18, semanticRadii=9, semanticSpacing=18, shadows=6, typography=18, light=55, dark=55, changedFiles=0
```
...
ไฟล์ generated ทุกไฟล์มี header `GENERATED CODE - DO NOT MODIFY BY HAND` การแก้ด้วยมือจะถูกเขียนทับในการ generate ครั้งถัดไปและทำให้ snapshot test ไม่ผ่าน

### `generated/v3_primitive_dimensions.g.dart` และ `generated/v3_semantic_dimensions.g.dart`

Primitive output ประกาศ `V3PrimitiveRadii`/`V3PrimitiveSpacing` ส่วน semantic output ประกาศ `V3Radii`/`V3Spacing` และอ้างกลับไปยัง primitive constants โดยตรง Widget ควรใช้ semantic classes ผ่าน `v3_dimensions.dart`

### `generated/v3_primitive_shadows.g.dart` และ `generated/v3_typography.g.dart`

Shadow output ประกาศ `V3PrimitiveShadows` เป็น `List<BoxShadow>` โดยรวม numbered layers ของ scale เดียวกัน ส่วน typography output ประกาศ `V3Typography` เป็น semantic `TextStyle` constants Widget ที่รองรับหลาย locale ควรตรวจ font fallback ของ `Noto Sans` และ override font family ตาม locale เมื่อจำเป็น

`Noto Sans` สำหรับ Theme V3 ถูก bundle แบบ deterministic/offline ใต้ `lib/assets/fonts/v3/` และ register เป็น family `Noto Sans` ใน `pubspec.yaml` ครบ weights 300, 400, 500, 600, 700 และ 800 เพื่อให้ชื่อ family ที่ generated typography tokens ระบุ resolve เป็น font asset จริงทั้งแอปและ Flutter Web preview; license อยู่ที่ `lib/assets/fonts/v3/OFL.txt`. ห้ามเปลี่ยนชื่อ family ฝั่ง asset registration หรือ token เพียงด้านเดียว

## Runtime Public API

### `v3_dimensions.dart`

เป็น public export boundary สำหรับ theme-invariant semantic dimensions:

```dart
const radius = V3Radii.roundedMd;
...
เมื่อทีม Design ส่ง token JSON ชุดใหม่มา **ไม่ต้อง generate JSON ซ้ำ** เพราะไฟล์จาก Figma เป็น editable source of truth โดยตรง ให้นำไฟล์ใน export batch มาแทน source ที่เกี่ยวข้องแล้ว generate Dart ใหม่ทุกครั้ง

ไฟล์ที่ต้อง export และตำแหน่งปลายทาง:

| Figma export | ตำแหน่งใน repo |
|---|---|
| `primitive.tokens.json` | `lib/config/themes/v3/tokens/primitive/primitive.tokens.json` |
| Primitive `color.alpha.json` | `lib/config/themes/v3/tokens/primitive/color.alpha.json` |
| Primitive `radius.json` | `lib/config/themes/v3/tokens/primitive/radius.json` |
| Primitive `space.json` | `lib/config/themes/v3/tokens/primitive/space.json` |
| Primitive `shadow.effect.json` | `lib/config/themes/v3/tokens/primitive/shadow.effect.json` |
...
| Semantic `radius.json` | `lib/config/themes/v3/tokens/semantic/radius.json` |
| Semantic `space.json` | `lib/config/themes/v3/tokens/semantic/space.json` |
| Semantic `typography.json` | `lib/config/themes/v3/tokens/semantic/typography.json` |

ชื่อไฟล์ export ต้องตรงกับชื่อ canonical ในตารางเท่านั้น โดยเปรียบเทียบแบบ case-sensitive:

- `primitive.tokens.json`
- `color.alpha.json`
- `radius.json` ใน primitive และ semantic folders
- `space.json` ใน primitive และ semantic folders
- `shadow.effect.json`
- `typography.json`
- `light.tokens.json`
- `dark.tokens.json`
...
ควรอัปเดต primitive และ semantic คู่ที่เกี่ยวข้องจาก export batch เดียวกัน เพื่อป้องกัน semantic aliases ชี้ primitive คนละ version ห้ามแก้ไฟล์ใต้ `generated/` ด้วยมือ

ขั้นตอนมาตรฐาน:

1. ตรวจชื่อไฟล์ export ให้ตรงกับชื่อ canonical แบบ case-sensitive และยืนยันว่าไม่มี suffix หรือชื่อซ้ำจากการดาวน์โหลด
2. เก็บ diff ของ token sources เดิมไว้ตรวจสอบ และยืนยันว่าไฟล์ใหม่เป็น JSON ที่อ่านได้
3. นำ Figma exports มาแทนตำแหน่ง source ด้านบน โดยคงชื่อและตำแหน่งปลายทางเดิม
...
1. แก้ HEX/alpha ที่ `tokens/primitive/primitive.tokens.json`
...
1. เพิ่ม token ใน `tokens/primitive/primitive.tokens.json`
