...
- `flutter-widget-v3-preview` (all 3 packs) has an explicit "Live Browser Preview (On Request)" flow so anyone using the Skills V3 pack can name a component and get a running browser preview without touching Widgetbook at all: resolve `lib/widgets/v3/<category>/preview_v3_<widget>.dart` (installing it first via `flutter-widget-v3-install` if not local yet), then run `flutter run -t <preview path> -d web-server --web-hostname 127.0.0.1 --web-port <port>` as a background process and hand back the served URL. This never touches `lib/widgetbook.dart`, `lib/widgetbook_use_cases.dart`, or `lib/widgetbook.directories.g.dart` since standalone `preview_*.dart` files are self-contained Flutter entrypoints. `flutter-widget-v3-search` hands off "show me this running" requests to this flow instead of launching anything itself. Verified live for `V3MiniButton` at `http://127.0.0.1:8090` via `flutter run -t lib/widgets/v3/button/preview_v3_mini_button.dart -d web-server --web-hostname 127.0.0.1 --web-port 8090`, confirmed with a real `curl` returning HTTP 200; the demo server was stopped afterward.
