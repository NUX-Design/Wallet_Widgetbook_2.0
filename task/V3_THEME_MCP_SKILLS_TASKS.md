...
อัปเดตล่าสุดเมื่อ: `2026-07-13 21:23:50 +0700`
...
- [x] สร้าง `lib/config/themes/v3/tokens/primitive/primitive.tokens.json`
...
- Source inputs: `lib/config/themes/v3/tokens/primitive/primitive.tokens.json`, `lib/config/themes/v3/tokens/semantic/light.tokens.json`, `lib/config/themes/v3/tokens/semantic/dark.tokens.json`
...
## Phase 8 — Primitive Token Expansion

### V3-26: Add alpha color, radius and spacing primitives

- [x] นำเข้า direct Figma JSON สำหรับ primitive alpha colors
- [x] นำเข้า direct Figma JSON สำหรับ primitive radii และ spacing
- [x] ขยาย generator ให้สร้าง alpha colors ใน `V3PrimitiveColors`
- [x] เพิ่ม number-token parser และ generated dimension APIs
- [x] เพิ่ม public barrel สำหรับ primitive APIs
- [x] เพิ่ม token count, parser, snapshot และ deterministic generation tests
- [x] ตรวจ V3 boundaries และ Flutter regression

Depends on: V3-10

Lane: Theme inputs / generator

Evidence (`2026-07-13 20:32:27 +0700`): source inputs ทั้งชุดอยู่ใต้ `lib/config/themes/v3/tokens/primitive/` ได้แก่ `primitive.tokens.json` 145 tokens, `color.alpha.json` 52 tokens, `radius.json` 9 tokens และ `space.json` 18 tokens; `V3ThemeGenerator` รวม alpha colors เข้า `V3PrimitiveColors` และ generate `V3PrimitiveRadii`/`V3PrimitiveSpacing` ใน `generated/v3_primitive_dimensions.g.dart`; public API export ผ่าน `v3_primitives.dart`; generator รอบสอง `changedFiles=0`; `flutter test test/config/themes/v3` PASS 20/20 และ full `flutter test` PASS 142/142; `flutter analyze` PASS; MCP V3 tests PASS 13/13 หลังอัปเดต catalog/fixture ไปยัง path ใหม่; `npm run check:v3-boundaries` PASS (61 Dart files, 21 changed paths); `npm run test:v3-boundaries` PASS 8/8. Semantic radius/spacing ยังไม่อยู่ใน scope ของงานนี้

### V3-27: Add semantic radius and spacing tokens

- [x] นำเข้า `tokens/semantic/radius.json` และ `tokens/semantic/space.json`
- [x] parse Figma alias metadata สำหรับ number tokens
- [x] resolve semantic dimensions ไปยัง primitive target
- [x] ตรวจ missing target และ resolved-value mismatch
- [x] generate `V3Radii` และ `V3Spacing`
- [x] เพิ่ม public semantic dimension barrel และ usage docs
- [x] เพิ่ม source count, snapshot, deterministic และ public API tests

Depends on: V3-26

Lane: Theme inputs / generator

Evidence (`2026-07-13 20:36:46 +0700`): direct Figma inputs `tokens/semantic/radius.json` 9 aliases และ `tokens/semantic/space.json` 18 aliases resolve กลับไป primitive ได้ครบ; `V3NumberTokenResolver` ตรวจ alias target และ resolved value; generator สร้าง `generated/v3_semantic_dimensions.g.dart` พร้อม public `V3Radii`/`V3Spacing` ผ่าน `v3_dimensions.dart`; generator รอบสอง `changedFiles=0`; targeted Theme V3 tests PASS 25/25 และ full `flutter test` PASS 148/148; `flutter analyze` PASS; V3 boundary checker PASS 64 Dart files/26 changed paths และ boundary tests PASS 8/8

### V3-28: Add semantic typography and primitive shadows

- [x] นำเข้า `tokens/semantic/typography.json`
- [x] นำเข้า `tokens/primitive/shadow.effect.json`
- [x] parse/validate font definition, weights และ typography metrics
- [x] แปลง absolute line-height เป็น Flutter height ratio
- [x] resolve shadow colors ไปยัง alpha primitives
- [x] compose numbered shadow layers เป็น 6 scales
- [x] generate `V3Typography` และ `V3PrimitiveShadows`
- [x] เพิ่ม public exports, docs, snapshots และ parser tests

Depends on: V3-26, V3-27

Lane: Theme inputs / generator

Evidence (`2026-07-13 20:59:44 +0700`): direct Figma `typography.json` มี 18 semantic styles และ generate เป็น `V3Typography`; `shadow.effect.json` มี 10 layers ซึ่ง resolve alpha aliases ครบและ compose เป็น `V3PrimitiveShadows` 6 scales; generator รอบสอง `changedFiles=0`; targeted Theme V3 tests PASS 29/29 และ full `flutter test` PASS 152/152; `flutter analyze` PASS; V3 boundary checker PASS 71 Dart files/36 changed paths และ boundary tests PASS 8/8. Typography parser ทดสอบ font alias/weight validation และ Shadow parser ทดสอบ incomplete layer rejection; generated snapshot/public API tests ครอบคลุม `headingMedium`, `md` และ `xl2`

### V3-29: Apply complete Theme V3 tokens to Mini Button

- [x] ตรวจซ้ำ 12 Figma Mini Button nodes ครบ 3 variants × 4 states
- [x] เปลี่ยน typography, spacing, radius, alpha และ shadow จาก magic numbers เป็น V3 token APIs
- [x] จำกัด `shadow-sm` ให้ Outline Default/Active ตาม Figma
- [x] เพิ่ม targeted token-wiring tests และอัปเดต widget guide/metadata
- [x] ตรวจ targeted/full Flutter tests, analyze และ V3 boundaries

Depends on: V3-28

Lane: Widget V3 / token adoption

Evidence (`2026-07-13 21:08:14 +0700`): เรียก Figma design context ซ้ำครบ nodes `241:3475`, `241:3479`, `241:3483`, `241:3487`, `241:3851`, `241:3855`, `241:3859`, `241:3847`, `241:3991`, `241:3995`, `241:3999`, `241:4003`; `V3MiniButton` ใช้ `V3Typography.labelTiny`, semantic `V3Spacing`/`V3Radii`, primitive alpha และ `V3PrimitiveShadows.sm` ตาม spec; targeted widget tests PASS 10/10, full `flutter test` PASS 153/153, `flutter analyze` PASS, V3 boundary checker PASS (71 Dart files, 38 changed paths) และ boundary tests PASS 8/8

### V3-30: Bootstrap a new Flutter app from V3 Beginner

- [x] เปลี่ยน `bootstrap-new` จาก stop-only เป็น confirmed Flutter project creation
- [x] เพิ่ม read-only `get_v3_theme_foundation` manifest/file delivery tool
- [x] รองรับ starter Widget V3, standalone preview, Light/Dark entrypoint และ tests
- [x] sync Codex/Claude Code/Kiro skill packs และ Codex UI metadata
- [x] เพิ่ม contract/integration/skill validators และ forward-test บน empty workspace

Depends on: V3-21, V3-28

Lane: Skills V3 / new-project bootstrap

Evidence (`2026-07-13 21:23:50 +0700`): `mcp-server/v3/foundation_catalog.js` allowlist runtime foundation 11 ไฟล์และ reject legacy/arbitrary paths; V3 contracts เพิ่มเป็น 18 tools โดย `get_v3_theme_foundation` เป็น read-only และ remote-safe; MCP syntax PASS 46 files, tests PASS 29/29 หลัง review/update additive contract snapshot และ local HTTP verifier PASS 28 read-only tools; `npm run validate:v3-skills` PASS 3 packs × 8 skills/18 tools พร้อม bootstrap-new markers; V3 boundaries PASS 71 Dart files/53 changed paths และ boundary tests PASS 8/8; forward-test สร้าง temp web app ด้วย `flutter create --project-name v3_starter --org com.wi --platforms web`, ติดตั้ง foundation + `V3MiniButton`, `flutter analyze` PASS และ `flutter test` PASS 1/1. Hosted Render endpoint ยังต้อง deploy commit ใหม่นี้ก่อน remote clients จะเห็น tool ลำดับที่ 18/remote read-only ลำดับที่ 28
...
7. `V3-26`: primitive alpha color, radius และ spacing expansion
8. `V3-27`: semantic radius และ spacing integration
9. `V3-28`: semantic typography และ primitive shadow integration
10. `V3-29`: apply complete Theme V3 tokens to Mini Button
11. `V3-30`: bootstrap a new Flutter app from V3 Beginner
